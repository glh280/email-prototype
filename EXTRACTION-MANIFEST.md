...
| `mock/inbox.ts` | — | — | modified | Iter: each row carries accountId + groupId for /inbox2 scoping. Iter (2026-04-27): expanded INBOX_ROWS from 13 → 30 rows so live-derived nav-rail / group / bell badges show realistic counts; covers all 5 deal tracks (Title, Lending, Deal Desk, Consulting, Partnership) across both accounts; spam +2, team +1. |
...
| `app/inbox2/_components/inbox2-sub-header.tsx` | new | Iter (2026-04-27): Missive realignment — search + filter chips moved to top bar. Title + thread count + Mark all read / Refresh / Sort actions. Iter (2026-04-27): drop flex-wrap; convert actions to icon-only buttons (with `title` tooltip) so the row no longer wraps at narrow center widths. |
...
| `app/inbox2/_components/inbox2-message-row.tsx` | new | Dense single-line row. Iter (2026-04-27): drop snippet from compact row (preview pane shows it); shrink sender col w-32 → w-24; group attachment + timestamp into one tight cluster (`gap-1`). |
| `app/inbox2/_components/inbox2-preview-pane.tsx` | new | Reader card. Iter (2026-04-27): drop `border-l` (divider owns line). Iter (2026-04-27): full email render — context bubble (Mailbox · File · Property), side-by-side AI summary + Snippet bubbles, per-message thread cards with from/to/cc/bcc + body + sentAt. |

| Path | Status | Notes |
|---|---|---|
| `mock/inbox2.ts` | new | Accounts, Groups (5 deal tracks), defaults, NAV_VIEW_LABEL. Iter (2026-04-27): Missive realignment — NAV_VIEW_LABEL keys rewritten for new union; DEFAULT_NAV_VIEW now "inbox". Iter (2026-04-27): static badge consts dropped — counts now derived live in Inbox2Shell from real INBOX_ROWS. |
